...
const TaskCard = ({ task, index, handleTaskChanged }) => {
  ...
  const updateTask = async () => {
    try {
      ...
      toast.error("Có lỗi xảy ra khi chỉnh sửa nhiệm vụ.");
    }
  };
  ...
  const toggleTaskCompleteButton = async () => {
    try {
      ...
      console.error("lỗi xảy ra khi update task toggle", error);
      toast.error("Lỗi xảy ra khi chỉnh sửa nhiệm vụ.");
    }
  };
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <Button
          ...
          size="icon"
          className={cn(
            "flex-shirk-0 size-8 rounded-full transition-all duration-200",
